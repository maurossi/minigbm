 ...
 */
#define NV_FORMAT_MOD_NVIDIA_BLOCK_LINEAR_2D(c, s, g, k, h) \
...
#define NV_GOB_HEIGHT_KIND_FERMI 0
#define NV_GOB_HEIGHT_KIND_G80 1
#define NV_GOB_HEIGHT_KIND_TURING 2

#define NV_GOB_SECTOR_LAYOUT_TEGRA 0
#define NV_GOB_SECTOR_LAYOUT_FERMI 1
#define NV_GOB_SECTOR_LAYOUT_BLACKWELL_8 2
#define NV_GOB_SECTOR_LAYOUT_BLACKWELL_16 3
...
struct nv_device_info {
	enum nv_bus_type bus_type;
	uint16_t chipset;
};

struct nv_device {
	enum nv_driver driver;
	struct nv_device_info info;
	int nouveau_fd;
};

static bool
nv_wants_tegra_display(const struct nv_device_info *devinfo)
{
	return devinfo->bus_type == NV_BUS_TYPE_SOC &&
	       devinfo->chipset < 0x14b /* Turing A */;
}

/* We only care about 2D images so we can just use +1 for the tiling */
#define NV_TILING(y_log2) ((y_log2) + 1)

#define NV_GOB_WIDTH_B 64
#define NV_GOB_HEIGHT 8

static uint32_t
nv_tiling_y_log2(uint32_t tiling)
{
	assert(tiling > 0);
	return tiling - 1;
}
...
static uint32_t
nv_tiling_block_width_B(uint32_t tiling)
{
	if (tiling == 0)
		return 1;

	return NV_GOB_WIDTH_B;
}

static uint32_t
nv_tiling_block_height(uint32_t tiling)
{
	if (tiling == 0)
		return 1;

	return NV_GOB_HEIGHT << nv_tiling_y_log2(tiling);
}

static bool
nv_tiling_valid(uint32_t tiling, uint32_t height)
{
	if (tiling == 0)
		return true;
	...
	return nv_tiling_block_height(tiling) < height * 2;
}

static uint16_t
nv_tiling_tile_mode(uint32_t tiling)
{
	if (tiling == 0)
		return 0;

	return nv_tiling_y_log2(tiling) << 4;
}

static uint8_t
nv_choose_pte_kind(struct nv_device *nvdev)
{
...
static uint64_t
nv_get_modifier(struct nv_device *nvdev,
		...
		uint32_t pte_kind)
{
	...
	const uint32_t y_log2 = nv_tiling_y_log2(tiling);

	/* The Tegra driver only supports the old modifiers */
	if (nvdev->driver == NV_DRIVER_TEGRA) {
	...
	if (nvdev->info.chipset >= 0x1a0 /* Blackwell A */) {
		gob_height_kind_gen = NV_GOB_HEIGHT_KIND_TURING;
		if (cpp == 1)
			sector_layout = NV_GOB_SECTOR_LAYOUT_BLACKWELL_8;
		else if (cpp == 2)
			sector_layout = NV_GOB_SECTOR_LAYOUT_BLACKWELL_16;
		else
			sector_layout = NV_GOB_SECTOR_LAYOUT_FERMI;
	} else if (nvdev->info.chipset >= 0x14b /* Turing A */) {
		gob_height_kind_gen = NV_GOB_HEIGHT_KIND_TURING;
		sector_layout = NV_GOB_SECTOR_LAYOUT_FERMI;
	} else if (nvdev->info.bus_type == NV_BUS_TYPE_SOC) {
		gob_height_kind_gen = NV_GOB_HEIGHT_KIND_FERMI;
		sector_layout = NV_GOB_SECTOR_LAYOUT_TEGRA;
	} else {
		gob_height_kind_gen = NV_GOB_HEIGHT_KIND_FERMI;
		sector_layout = NV_GOB_SECTOR_LAYOUT_FERMI;
	}

	return NV_FORMAT_MOD_NVIDIA_BLOCK_LINEAR_2D(0 /* Uncompressed */,
						    sector_layout,
						    gob_height_kind_gen,
						    pte_kind, y_log2);
}
...
static uint32_t
nv_modifier_to_tiling(uint64_t modifier)
{
	if (modifier == DRM_FORMAT_MOD_LINEAR)
		return 0;

	const uint32_t y_log2 = modifier & 0xf;
	return NV_TILING(y_log2);
}

static uint64_t
nv_shrink_modifier(uint64_t modifier, uint32_t height)
{
	if (modifier == DRM_FORMAT_MOD_LINEAR)
		return DRM_FORMAT_MOD_LINEAR;
	...
	while (true) {
		...
		if (nv_tiling_valid(tiling, height))
			return modifier;

		assert(tiling != NV_TILING(0));
		modifier--;
	}
...
static bool
nv_is_modifier_better(uint64_t a_mod, uint64_t b_mod)
{
	...
	const uint32_t a_tiling = nv_modifier_to_tiling(a_mod);
	const uint32_t b_tiling = nv_modifier_to_tiling(b_mod);

	return a_tiling > b_tiling;
}

static uint64_t
nv_choose_modifier_from_list(struct driver *drv,
			     uint32_t format, uint32_t height,
			     const uint64_t *modifiers,
			     uint32_t modifier_count)
{
	struct nv_device *nvdev = drv->priv;
	const uint8_t pte_kind = nv_choose_pte_kind(nvdev);
	...
	for (uint32_t i = 0; i < modifier_count; i++) {
		const uint32_t tiling = nv_modifier_to_tiling(modifiers[i]);

		/* Re-create the modifier to ensure it's the right one for our
		 * device.
		 */
		const uint64_t modifier =
			nv_get_modifier(nvdev, cpp, tiling, pte_kind);
		...
		if (nv_is_modifier_better(modifier, best_modifier))
...
static void
nv_calculate_layout(struct bo *bo, uint64_t modifier)
{
	const uint32_t width = bo->meta.width;
	const uint32_t height = bo->meta.height;
	const uint32_t stride =
	    drv_stride_from_format(bo->meta.format, width, 0);
	const uint32_t tiling = nv_modifier_to_tiling(modifier);
	...
	if (tiling == 0) {
		...
		stride_align = nv_tiling_block_width_B(tiling);
...
static enum nv_driver
nv_fd_get_driver(int fd)
{
	enum nv_driver driver = NV_DRIVER_UNKNOWN;
...
static int
nv_open_nouveau_device(struct nv_device *nvdev)
{
...
static int
nouveau_get_device_info(int fd, struct nv_device_info *devinfo)
{
	int err;
...
nouveau_has_display(int fd)
{
	...
	return !nouveau_get_device_info(fd, &devinfo) &&
	       !nv_wants_tegra_display(&devinfo);
}
...
static void
nv_add_format_combinations(struct driver *drv, uint32_t format,
			   uint64_t use_flags)
{
	struct nv_device *nvdev = drv->priv;
	const uint8_t pte_kind = nv_choose_pte_kind(nvdev);
	...
	for (uint32_t y_log2 = 0; y_log2 <= 5; y_log2++) {
		const uint32_t tiling = NV_TILING(y_log2);
		const uint64_t modifier =
			nv_get_modifier(nvdev, cpp, tiling, pte_kind);
...
nv_add_combinations(struct driver *drv)
{
	...
	for (uint32_t i = 0; i < ARRAY_SIZE(texture_source_formats); i++) {
		nv_add_format_combinations(drv, texture_source_formats[i],
					   texture_use_flags & ~sw_flags);
	}
}

static struct nv_device *
nv_device_alloc(enum nv_driver driver)
{
	struct nv_device *nvdev;
...
static void
nv_device_destroy(struct nv_device *nvdev)
{
...
static void
nv_close(struct driver *drv)
{
	nv_device_destroy(drv->priv);
	drv->priv = NULL;
}
...
nouveau_init(struct driver *drv)
{
	...
	nvdev = nv_device_alloc(NV_DRIVER_NOUVEAU);
	if (nvdev == NULL)
		return -ENOMEM;
	...
	drv->priv = nvdev;
	nv_add_combinations(drv);

	return 0;

fail:
	nv_device_destroy(nvdev);
	return err;
}

static int
nv_get_nouveau_fd(struct driver *drv)
{
	struct nv_device *nvdev = drv->priv;
	if (nvdev->nouveau_fd >= 0)
			       ...
			       uint32_t format, const uint64_t modifier)
{
	...
	uint32_t domain = 0;
	if (nvdev->info.bus_type == NV_BUS_TYPE_SOC) {
	...
	int err = drmCommandWriteRead(nv_get_nouveau_fd(bo->drv),
				      DRM_NOUVEAU_GEM_NEW,
				      &req, sizeof(req));
	if (err) {
...
nouveau_bo_map(struct bo *bo, struct vma *vma, uint32_t map_flags)
{
	...
	return mmap(0, bo->meta.total_size, drv_get_prot(map_flags),
		    MAP_SHARED, bo->drv->fd, info.map_handle);
}
...
tegra_init(struct driver *drv)
{
	...
	nvdev = nv_device_alloc(NV_DRIVER_TEGRA);
	if (nvdev == NULL)
		return -ENOMEM;

	err = nv_open_nouveau_device(nvdev);
	if (err)
		goto fail;
...
static int
tegra_bo_create_for_modifier(struct bo *bo, uint32_t width, uint32_t height,
			     uint32_t format, const uint64_t modifier)
{
	struct nv_device *nvdev = bo->drv->priv;
	int err;
	...
	if (modifier == DRM_FORMAT_MOD_LINEAR) {
		nv_calculate_layout(bo, DRM_FORMAT_MOD_LINEAR);
		 ...
		 */
		err = nouveau_bo_create_for_modifier(bo, width, height,
						     format, modifier);
		if (err)
			return err;
...
static int
tegra_bo_create_with_modifiers(struct bo *bo, uint32_t width,
			       uint32_t height, uint32_t format,
			       const uint64_t *modifiers,
			       uint32_t modifier_count)
{
	...
	return tegra_bo_create_for_modifier(bo, width, height, format, modifier);
}

static int
tegra_bo_create(struct bo *bo, uint32_t width, uint32_t height,
	        uint32_t format, uint64_t flags)
{
	struct combination *combo = drv_get_combination(bo->drv, format, flags);
	...
	return tegra_bo_create_for_modifier(bo, width, height,
					    format, modifier);
}
...
const struct backend backend_tegra = {
	...
	.bo_create_with_modifiers = tegra_bo_create_with_modifiers,
	.bo_destroy = drv_gem_bo_destroy,
	.bo_import = drv_prime_bo_import,
	.bo_map = tegra_bo_map,
	.bo_unmap = drv_bo_munmap,
